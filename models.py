...
class Net(nn.Module):
    def __init__(self):
        ...
        # Final linear layer for 68 keypoints * 2 coordinates
    ...
    def _initialize_weights(self):
        # Use activation-aware gain: ReLU for hidden layers, 1.0 for final linear output
        relu_gain = nn.init.calculate_gain('relu')  # ~sqrt(2)

        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # Conv layers are followed by ReLU -> use relu gain
                ...
                # fc2 is the output (linear regression) -> use gain=1.0
                gain = 1.0 if m is self.fc2 else relu_gain
    ...
    def forward(self, x):
        ...
        x = self.fc2(x)  # Linear output for keypoint coordinates
